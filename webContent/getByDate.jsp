...
<form name="task_id_form" action="getByDate.jsp" method="post">

    <%
        try {
            String query = "//task";

            List<String> taskDates = TasksJDOMParser.getTaskDates(xmlStream);
            for (String taskDate : taskDates) {
    %>        	
    <input type="radio" name="task_date" value="<%=taskDate.replace("-", "") %>"><%=taskDate %><br>
    <%
            }
...
<textarea id="txtAreaTaskXml" cols="100" rows="30">
    ...
    <%
        ...
        try {
            ...
            String query = "//task[number(translate(@date,'-',''))="+request.getParameter("task_date").toString()+"]";
